...
const requireAdmin = (req, res, next) => {
  const headerPass =
    ...
    req.headers["x-admin"];
  const queryPass = req.query?.adminPass || req.query?.admin_pass;
  const bodyPass = req.body?.adminPass || req.body?.admin_pass;
  const password = sanitizeText(headerPass || queryPass || bodyPass);
